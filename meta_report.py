...
"""
def plot_scaffold(x_data,y_data,title,xlab,ylab,file_name,pattern_lengths):
    ...
    for i in range(len(pattern_lengths)):
        plt.plot(x_data[sum(pattern_lengths[:i]):sum(pattern_lengths[:i+1])],y_data[sum(pattern_lengths[:i]):sum(pattern_lengths[:i+1])],'o',label='Pattern %s' % (i+1))
            ...
            for l in range(len(plot_labels)):
                if ('%s vs %s' % (plot_labels[j][0],plot_labels[l][0][:15])) in all_plots:
                    ...
                    plot_scaffold(x_data,y_data,'%s vs %s' % (plot_labels[j][0],plot_labels[l][0]),plot_labels[j][0],plot_labels[l][0],('%s vs %s' % 
                        (plot_labels[j][0],plot_labels[l][0])).replace(' ','_')+'.png',pattern_lengths)
                    break
